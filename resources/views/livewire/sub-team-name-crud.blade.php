        ...
        <div class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div class="bg-white p-6 rounded shadow w-full max-w-lg">
                ...
                <table class="w-full mb-4 text-sm">
                    ...
                    <tbody>
                        @foreach($subTeamNames as $sub)
                            <tr class="border-b">
                                <td class="py-2">{{ $sub->name }}</td>
                                <td class="py-2 text-right">
                                    <button type="button" class="text-xs text-blue-600 hover:underline mr-2" wire:click="showEditModal({{ $sub->id }})">Editar</button>
                                    <button type="button" class="text-xs text-red-600 hover:underline" wire:click="confirmDelete({{ $sub->id }})">Borrar</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
