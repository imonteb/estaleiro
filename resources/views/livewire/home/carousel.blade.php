<div>
    ...
    <div id="default-carousel" class="relative  m-auto" data-carousel="slide">
        ...
        <div class="relative h-50 overflow-hidden rounded-lg md:h-96">
            @foreach ($carouselItems as $imagen)
            <div class="{{ $loop->first ? 'block' : 'hidden' }} duration-700 ease-in-out" data-carousel-item wire:key="imagen-{{ $loop->index }}">
                <img src="{{ $imagen['image'] }}" class="absolute block w-full" alt="{{ $imagen['title'] }}">
            </div>
            @endforeach
        </div>
        ...
        <div class="absolute z-30 flex space-x-3 -translate-x-1/2 bottom-5 left-1/2">
            @foreach ($carouselItems as $index => $imagen)
                <button type="button"
